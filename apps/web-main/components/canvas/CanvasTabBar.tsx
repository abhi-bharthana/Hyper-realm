...
export function CanvasTabBar({ pages, activePageId, setActivePageId, onAddPage, onRenamePage, onDeletePage, isLight }: Props) {
  // 🚀 NAYA: Search aur Expand/Collapse States
  ...
  // 🚀 SEARCH LOGIC: Filter nodes based on search query
  const filteredPages = useMemo(() => {
    ...
    // To show a matching sub-node, we MUST include its parents too
  ...
  const renderTree = (parentId: string | null, depth: number = 0) => {
    ...
    return children.map((p) => {
      ...
      // Agar search chal rahi hai, toh sab kuch auto-expand rakho, warna local state use karo
          ...
          >
            ...
            <div className="flex items-center gap-1.5 truncate">
              ...
              {hasChildren ? (
                ...
                <div className="w-4.5 h-4.5" /> /* Empty space for alignment */
              )}
            ...
            {/* RIGHTSIDE: Counters & Hover Actions */}
            <div className="flex items-center gap-1.5 shrink-0 ml-2">
              {/* Child Counter (Only shows if node has children & is collapsed, or on hover) */}
  ...
  return (
    <div className="fixed left-6 top-1/2 -translate-y-1/2 z-40 flex flex-col pointer-events-none animate-in slide-in-from-left-8 duration-700 ease-[cubic-bezier(0.23,1,0.32,1)]">
      <div className={`pointer-events-auto flex flex-col items-start p-2.5 rounded-[28px] backdrop-blur-2xl border shadow-2xl transition-all max-h-[85vh] w-64
        ${isLight ? 'bg-white/70 border-white/50 shadow-slate-200/50' : 'bg-[#111111]/80 border-white/10 shadow-black/50'}
      `}>
        ...
        <div className="w-full px-2 mt-1 mb-3 relative">
          ...
          <input 
            ...
            className={`w-full pl-8 pr-3 py-2 rounded-xl text-xs font-bold tracking-wide outline-none transition-all placeholder:font-medium
              ${isLight 
                ? 'bg-slate-100/50 focus:bg-white text-slate-700 placeholder:text-slate-400 border border-transparent focus:border-primary/30' 
                : 'bg-white/5 focus:bg-black/50 text-zinc-300 placeholder:text-zinc-600 border border-transparent focus:border-primary/30'}`}
          />
        </div>

        {/* TAB ITEMS (Recursive Tree) */}
        <div className="flex flex-col w-full relative overflow-y-auto custom-scrollbar flex-1 px-1 min-h-[100px]">
          {pages.length === 0 ? (
            <div className="text-center w-full py-4 text-xs font-black tracking-widest text-zinc-500 uppercase opacity-50">Empty</div>
          ) : (
            renderTree(null, 0)
          )}
        </div>
        ...
        <button 
          ...
          className={`w-full py-3 rounded-2xl transition-all shrink-0 active:scale-95 group flex items-center justify-center gap-2 text-[11px] font-black uppercase tracking-widest
            ${isLight ? 'hover:bg-primary/10 text-slate-500 hover:text-primary' : 'hover:bg-primary/20 text-zinc-500 hover:text-primary'}`}
        >
